...
public class TrafficLoadService {
	...
	public TrafficLoadBean getTrafficLoadNew(TrafficBean trafficModel) throws Exception {
		TrafficLoadBean loadBean = new TrafficLoadBean();
		String stEnterpriseId = trafficModel.getEnterpriseId();
		// Null or Empty Check - EnterpriseId
		if (Strings.isNullOrEmpty(stEnterpriseId)) {
		...
		if (dtFromDate != null && dtToDate != null) {
			loadBean = dbOperations.getTrafficLoad1(stEnterpriseId, dtFromDate.getTime(), dtToDate.getTime(), inIncrementValue);
		} else if (dtFromDate != null) {
			// Set current time for todate
			loadBean = dbOperations.getTrafficLoad1(stEnterpriseId, dtFromDate.getTime(), new Date().getTime(), inIncrementValue);
		} else if (dtToDate != null) {
			// Set from date to 0
			Calendar cal = Calendar.getInstance();
			cal.setTime(dtToDate);
			cal.add(Calendar.DATE, 1);
			cal.add(Calendar.SECOND, -1);
			loadBean = dbOperations.getTrafficLoad1(stEnterpriseId, 0, cal.getTimeInMillis(), inIncrementValue);
		} else {
			loadBean.setResponseCode("2");
			loadBean.setResponseText("Request should contain either FromTime or ToTime");
			return loadBean;
